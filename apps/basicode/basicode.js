...
4 rem Modifications: call &bbde; no hidden machine code (copychr$ instead); HG: 270->276
...
22 poke SCRMODE,1: IF PEEK(SCRMODE)=1 THEN HO=39:VE=24:HG=276:VG=200:WINDOW #1,1,40,25,25
24 SV=7:OG=0:reading=&9E3F:writing=&9E00:GOTO 1009
100 CLS:RETURN
110 IF HO<0 THEN 115
111 IF VE<0 THEN 116
112 IF HO>39 AND HG<>552 THEN 117
...
224 LOCATE INT(HO+1),INT(VE+1): IN$=copychr$(#0):pen 0:paper 1:IN$=IN$+copychr$(#0):pen 1:paper 0:IN=ASC(UPPER$(IN$)): 'CALL &179:IN=ASC(UPPER$(CHR$(PEEK(&177))))
...
599 RETURN
600 CLG:ORIGIN 42,399:MOVE 0,0:RETURN
601 IF HO<0 THEN OG$="HO too low!":OG=1 ELSE IF HO>=1 THEN OG$="HO too high!":OG=1
602 IF VE<0 THEN OG$="VE too low!":OG=1 ELSE IF VE>=1 THEN OG$="VE too high!":OG=1
603 'IF HO=1 OR HO>1 THEN OG$="HO too high!":OG=1
604 'IF VE=1 OR VE>1 THEN OG$="VE too high!":OG=1
605 RETURN
620 GOSUB 601:IF OG=1 THEN 632
621 PLOT INT(HO*552),-INT(VE*400),ABS(CN-1):RETURN
622 'PRINT #1,OG$:SOUND 2,100,10,15:OG=0:RETURN
630 GOSUB 601:IF OG=1 THEN 632
631 DRAW INT(HO*552),-INT(VE*400),ABS(CN-1):RETURN
632 move INT(HO*552),-INT(VE*400): PRINT #1,OG$:SOUND 2,100,10,15:OG=0:RETURN
650 GOSUB 601:TAG:MOVE INT(HO*552),-INT(VE*400)

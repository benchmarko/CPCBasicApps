...
500 REM bmbench08 - BM Bench - bmbench.bas (BASIC) mod1
510 REM https://github.com/benchmarko/BMbench
600 REM PRINT "Press and release Space to start measurement"
610 REM IF INKEY(47)<>0 THEN 610
620 REM IF INKEY(47)<>-1 THEN 620
700 MODE 2
...
829 REM
830 CLEAR
831 prgLanguage$ = "Basic"
832 prgVersion$ = "0.08"
833 startTs = 0
834 maxBench = 6
835 DEF FNmod1(x,q) = x - INT(x / q) * q: REM x MOD q
...
2214 num1 = line1(j)
2215 line1(j) = FNmod1(line1(j) + prev1, 65536)
2216 prev1 = num1
2217 NEXT j
2218 line1(1) = i
2219 NEXT i
...
2224 xHelp = line1(j)
2225 xHelp = FNmod1(2.0 * (xHelp * xHelp), 65536)
2226 x = FNmod1(x + xHelp, 65536)
2227 NEXT j
2228 REM x = (x + line1(k) * line1(k)) AND 65535
2229 xHelp = line1(k)
2230 xHelp = FNmod1(xHelp * xHelp, 65536)
2231 x = FNmod1(x + xHelp, 65536)
2232 n = b05nSave
2233 RETURN
2234 REM
2235 REM
...
2300 check = -1
2301 IF bench = 0 THEN check = FNmod1((n / 2) * (n + 1), 65536)
...
2411 RETURN
2412 REM
2413 REM checkbitsFloat()
3000 num = 1
3001 lastNum = 0
3002 bits = 0
3003 WHILE (((num - 1) / 2) = lastNum) AND (bits < 101)
3004 lastNum = num
3005 num = num * 2
3006 num = num + 1
3007 bits = bits + 1
...
3010 REM
3011 REM
3012 REM printInfo(): void
3400 GOSUB 3000
3401 PRINT "BM Bench v"; prgVersion$; " ("; prgLanguage$; ") -- (float:"; bits; "tsMs:"; gtsPrecMs; "tsCnt:"; gtsPrecCnt; ") -- "; basicver$
3402 PRINT "(c) Marco Vieth, 2002-2023"
3403 RETURN
3404 REM
3405 REM
3406 REM printResult(): void
